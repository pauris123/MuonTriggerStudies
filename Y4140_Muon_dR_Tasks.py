...
for aiziet in range(notik,tik):
    ...
    # So the list might go - (10th L1T muon -> 1st GenP muon), (24th L1T muon -> 2nd GenP muon) and so on, so have to follow the iev and i numbers of L1T muon
        ...
        for i,genP in enumerate(genParticles.product()):
            if (genP.pdgId() == 20443 and genP.numberOfDaughters() == 2) : # Takes Mother particle that has 2 daughters
                ...
                if genP.daughter(0).numberOfDaughters() > 0: # Cheks the grand-daughters
                    ...
                    muon_eta_for_genP_dR_0 = [] # Lists for calculating dR between both muons that come from 1st daughter decay in GenParticles (should be J/Psi)
                    ...
                    if len(muon_eta_for_genP_dR_0) == 2: # Check whether there is Di-muon pair from J/Psi to calculate dR for, from GenP
                        di_muon_dR_JPsi_genP.append(((muon_eta_for_genP_dR_0[0]-muon_eta_for_genP_dR_0[1])**2+(muon_phi_for_genP_dR_0[0]-muon_phi_for_genP_dR_0[1])**2)**0.5)
                        ...
                        di_muon_invM_o_dR_JPsi_genP.append(((muon1_JPsi_genP+muon2_JPsi_genP).M()/((muon_eta_for_genP_dR_0[0]-muon_eta_for_genP_dR_0[1])**2+(muon_phi_for_genP_dR_0[0]-muon_phi_for_genP_dR_0[1])**2)**0.5))
                ...
                if genP.daughter(1).numberOfDaughters() > 0: # Checks all the same things for the 2nd daughter and following grand-daughters
                    ...
                    if len(muon_eta_for_genP_dR_1) == 2:
                        di_muon_dR_Phi_genP.append(((muon_eta_for_genP_dR_1[0]-muon_eta_for_genP_dR_1[1])**2+(muon_phi_for_genP_dR_1[0]-muon_phi_for_genP_dR_1[1])**2)**0.5)
                        ...
                        di_muon_invM_o_dR_Phi_genP.append(((muon1_Phi_genP+muon2_Phi_genP).M()/((muon_eta_for_genP_dR_0[0]-muon_eta_for_genP_dR_0[1])**2+(muon_phi_for_genP_dR_0[0]-muon_phi_for_genP_dR_0[1])**2)**0.5))                
    

            
    
        #Calculating the dR values for each genP muon matched with every L1T muon
        if (len(muon_pdgId) == 4 and len(muon_charge) >= 4) :  # (len(muon_pdgId) == 2 or len(muon_pdgId) == 4)
            #if len(muon_charge) == 2 and sum(muon_charge) != 0:
                #continue
            #if len(muon_charge) == 3 and abs(sum(muon_charge)) == 3:
                #continue
            if len(muon_charge) == 4 and abs(sum(muon_charge)) == 4 and abs(sum(muon_charge)) == 2:
                continue
            if len(muon_charge) == 5 and abs(sum(muon_charge)) == 5 and abs(sum(muon_charge)) == 3:
                continue
            if len(muon_charge) == 6 and abs(sum(muon_charge)) == 6 and abs(sum(muon_charge)) == 4:
                continue
            ...
            for j in range(len(muon_pdgId)): # Take the genP muon
                ...
                if j == 0 or j == 1: # J/Psi Matching
                
                    muon_dR_min_JPsi.append(min(muon_dR_min_calc))  # Take the min value of dR and add it to the list for matched muon dR's
                    muon_dR_min_matched_JPsi.append(min(muon_dR_min_calc))
                    muon_matched_JPsi_L1T_number.append(muon_dR_min_calc_fix.index(min(muon_dR_min_calc)))      
                   # Find the index of the L1T muon, that gave the lowest dR value for the particular genP muon, and add it to the extra list.
                    ...
                    muon_dR_min_JPsi_genP_i.append([muon_i_gen[j]])
                
                if j == 2 or j == 3: # Phi Matching
                
                    muon_dR_min_Phi.append(min(muon_dR_min_calc))  # Take the min value of dR and add it to the list for matched muon dR's
                    muon_dR_min_matched_Phi.append(min(muon_dR_min_calc))
                    muon_matched_Phi_L1T_number.append(muon_dR_min_calc_fix.index(min(muon_dR_min_calc)))      
                    # Find the index of the L1T muon, that gave the lowest dR value for the particular genP muon, and add it to the extra list.
                    muon_dR_min_Phi_iev.append(iev)
                    muon_dR_min_Phi_genP_i.append([muon_i_gen[j]])
            ...
            if len(muon_pdgId) == (len(muon_matched_JPsi_L1T_charge)+len(muon_matched_Phi_L1T_charge)): # Checking whether all genP muons got matched with something. 
                ...
                if len(muon_matched_JPsi_L1T_charge) == 2: # Calculating the dR and invariant masses for L1T muon pairs
                    ...
                    di_muon_dR_matched_L1T_JPsi.append(((muon_matched_JPsi_L1T_eta[0]-muon_matched_JPsi_L1T_eta[1])**2+(muon_matched_JPsi_L1T_phi[0]-muon_matched_JPsi_L1T_phi[1])**2)**0.5)
                    inv_M_o_dR_di_muon_JPsi.append(((muon1_JPsi+muon2_JPsi).M()/((muon_matched_JPsi_L1T_eta[0]-muon_matched_JPsi_L1T_eta[1])**2+(muon_matched_JPsi_L1T_phi[0]-muon_matched_JPsi_L1T_phi[1])**2)**0.5))
                else:
                    print("Something is wrong with 'muon_matched_JPsi_L1T_charge' list")
                ...
                if len(muon_matched_Phi_L1T_charge) == 2: # Calculating the dR and invariant masses for L1T muon pairs
                    ...
                    di_muon_dR_matched_L1T_Phi.append(((muon_matched_Phi_L1T_eta[0]-muon_matched_Phi_L1T_eta[1])**2+(muon_matched_Phi_L1T_phi[0]-muon_matched_Phi_L1T_phi[1])**2)**0.5)
                    inv_M_o_dR_di_muon_Phi.append(((muon1_Phi+muon2_Phi).M()/((muon_matched_Phi_L1T_eta[0]-muon_matched_Phi_L1T_eta[1])**2+(muon_matched_Phi_L1T_phi[0]-muon_matched_Phi_L1T_phi[1])**2)**0.5))
                else:
                    print("Something is wrong with 'muon_matched_Phi_L1T_charge' list")
    ...
    for b in range(len(di_muon_dR_JPsi_genP)):
        fon.write(str(di_muon_dR_JPsi_genP[b])+"\t"+str(di_muon_dR_Phi_genP[b])+"\t"+str(di_muon_dR_JPsi_genP_iev[b])+"\t"+str(aiziet)+"\n")
    fon.close()
    
    fob=open("Muon_dR_from_Matching_genP_L1T_JPsi_Phi_Y.txt","a")
    for h in range(len(muon_dR_min_JPsi)):
        fob.write(str(muon_dR_min_JPsi[h])+"\t"+str(muon_dR_min_Phi[h])+"\t"+str(aiziet)+"\n")
    fob.close()
